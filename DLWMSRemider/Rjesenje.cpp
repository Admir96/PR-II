...
public:
	...
	void SetElement2(int lokacija,bool oznacen)
	{
		if (lokacija < 0 || lokacija >= _trenutnoElemenata)
			throw Izuzetak("Nepostojeca lokacija", __FUNCTION__);
		_elementi2[lokacija] = oznacen;
	}
...
public:
	...
	bool AddObavezu(string obaveza)
	{
		...
		{
			if (_obaveze->GetElement1(i) == obaveza || regex_search(obaveza,regex("ubiti|UBITI|mrziti|MRZITI")))
				return false;
		}
...
public:
	...
	bool AddDogadjaj(Dogadjaj obj)
	{
		for (size_t i = 0; i < _dogadjaji.size(); i++)
		{
			if (strcmp(_dogadjaji[i].GetNaziv(),obj.GetNaziv()) == 0)
				return false;
		}
...
public :
	void AddStudent(Student obj)
	{
		...
		{
			if (_reminiderList[i].GetImePrezime() == obj.GetImePrezime() && _reminiderList[i].GetIndeks() == obj.GetIndeks())
				throw Izuzetak("nije omoguceno dupliranje !", __FUNCTION__);
		}
		_reminiderList.push_back(obj);
	}
	bool OznaciObavezuKaoZavrsenu(int index,string naziv,string obaveza)
	{
		for (vector<Student>::iterator i = _reminiderList.begin();i!= _reminiderList.end();i++)
		{
			if (i->GetIndeks() == index)
			{
				...
				{
					if (strcmp(j->GetNaziv(), naziv.c_str()) == 0)
					{
						for (size_t k = 0; k < j->GetObaveze()->GetTrenutno(); k++)
						{
							if (j->GetObaveze()->GetElement1(k) == obaveza && j->GetObaveze()->GetElement2(k) == false)
							{
...
void main() {
	...
	*/
	...
	2.Samostalno vjezbati
	Predlazemo Vam da ispunite i ostale planirane obaveze.
	FIT Reminder PosaljiNotifikacije
	-------------------------------------------------------------------------
