...
public:
	...
	Poglavlje(const Poglavlje& obj):_ocjena(obj._ocjena),_prihvaceno(obj._prihvaceno)
	{
	...
	void SetSadrzaj(string sadrzaj) {
	  char * temp = AlocirajNizKaraktera((string(_sadrzaj) + " " + sadrzaj).c_str());
	  delete[] _sadrzaj;
...
public:
	...
	void DodajPoglavlje(const char* nazivPoglavlja, const char * sadrzajPoglavlja)
	{
		for (vector<Poglavlje>::iterator i = _poglavljaRada.begin(); i != _poglavljaRada.end(); i++)
		{
			if (strcmp(i->GetNaslov(), nazivPoglavlja) == 0)
			{
				i->SetSadrzaj(string(sadrzajPoglavlja));
				return;
			}
		}
		Poglavlje* temp = new Poglavlje(nazivPoglavlja,sadrzajPoglavlja);
...
public:
	...
	bool DodajZavrsniRad(string index, ZavrsniRad rad)
	{
		try
		{
		  _teme.AddElement(index, rad);
		   return true;
		}
		catch (exception& err)
		{
	...
	{
		c << "ime nastavnika : " << obj._imePrezime << endl;
		cout << obj._teme << endl;
		return c;
	}
...
void main() {
	...
	//nazivPoglavlja, sadrzajPoglavlja DodajPoglavlje
	...
	//brojIndeksa, zavrsniRadDodajZavrsniRad
...
//brojIndeksa, datumOdbrane
	...
	da su svoj zavrsni rad uspjesno odbranili sa vecom ili manjom ocjenom od prosjecne.PosaljiPozivZaDodjeluNagrada
